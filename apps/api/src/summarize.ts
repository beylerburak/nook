 ...
 * - **The model's words, not the user's.** Which is why the server does not
 *   write the summary anywhere — see the note on `summarizeRecords`.
 */
...
/**
 * Why a bookmark did not get a summary. Machine-readable on purpose: the client
 * has to tell "nothing to do" (`too-short`, `already-summarised`, `not-found`)
 * from "it tried and could not" (`empty-output`, `failed`, `unavailable`), or a
 * throttled key looks exactly like a library with nothing worth summarising.
 */
 ...
 */
const MIN_SUMMARISABLE_CHARS = 400;
...
/**
 * Records summarised per route call. A caller is a background runner, so this
 * is a wall-clock bound rather than a billing one — the doc's cost table puts
 * the whole library at fractions of a cent — and a request above it is answered
 * 400 rather than silently truncated, so a client bug cannot look like a
 * finished batch.
 */
const MAX_IDS_ACCEPTED = 50;
...
function hasSummary(record: SummaryRecord): boolean {
...
/** Structural validation only, in the style of `parseClassifyRequest`: a thrown
 *  Error becomes a 400 whose message is safe to hand back to the client. */
export function parseSummarizeRequest(value: unknown): SummarizeRequest {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new Error("Invalid request");
  const body = value as Record<string, unknown>;
  if (!Array.isArray(body.ids)) throw new Error("Invalid ids array");
  if (body.ids.length > MAX_IDS_ACCEPTED) throw new Error("Too many ids");
  const ids: string[] = [];
  const seen = new Set<string>();
  for (const entry of body.ids) {
 ...
 * back as `deleted` instead of `not-found` — different answers, and the client
 ...
 *
 * **The server does not write `summary` anywhere, and that is the design.**
 *
 * It looks like a missed optimisation: the content is right here, the model has
 * just produced the text, and a `data = jsonb_set(...)` would make it visible
 * immediately. It is not, for three reasons that a write would break:
 *
 * - A summary is user-visible prose on a bookmark, not derived data like a
 *   vector. The client owns it: it shows it, the user can delete or re-run it,
 *   and signing out has to take it with everything else on that device.
 * - Going through `POST /api/sync` is what puts the summary on *every* device. A
 *   server-side write would have to invent a second push path, and the device
 *   that asked for the summary is the one holding the record the merge is
 *   computed over — so it would still write it locally anyway.
 * - A write here would move the record's version, and every other device would
 *   pull a change it never made, with a `updatedAt` that says a human edited a
 *   bookmark they did not touch. It would also break the merge rule this field
 *   exists to get: newer-wins on `summary` itself (see the note on the field in
 *   apps/extension/lib/types.ts), which only runs if the client is the author.
 *
 ...
 * at all, and the route should answer 500 rather than a confident empty batch.
 */
